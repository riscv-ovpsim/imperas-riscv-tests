...
void riscvDoc(riscvP rootProcessor) {
    ...
    if(cfg->arch&ISA_K) {
        ...
        ////////////////////////////////////////////////////////////////////////
        // CRYPTOGRAPHIC EXTENSION BUS PORT INTERFACE
        /////////////////////////////////////////////////////////////////////////

        {
            vmiDocNodeP External = vmidocAddSection(
                Crypto, "Cryptographic Extension Bus Port Interface"
            );

            vmidocAddText(
                External,
                "Bus \"entropy\" can be optionally connected to a component "
                "supplying entropy values for the \"pollentropy\" instruction. "
                "If connected, a 4-byte read of address 0 of this bus is "
                "performed when the instruction is executed and the 32-bit "
                "value read is assigned to the result register. If bus "
                "\"entropy\" is not connected, executions of \"pollentropy\" "
                "will return a repeatable sequence of ES16 values generated by "
                "an LFSR."
            );
        }
    }
...

    {
        ...
        if(cfg->arch&ISA_K) {
            snprintf(
                SNPRINTF_TGT(string),
                "RISC-V \"K\" Cryptographic Extension (0.5.0)"
            );
            vmidocAddText(References, string);
        }
